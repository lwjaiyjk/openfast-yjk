...
public class Group extends Field {
     ...
     * @param pmapReader The BitVector to be decoded
     * @param context    The previous object to keep the data in sync
    ...
    public FieldValue[] decodeFieldValues(InputStream in, Group template, BitVectorReader pmapReader, Context context) {
        ...
        for (int fieldIndex = start; fieldIndex < fields.length; fieldIndex++) {
            ...
            // 根据style进行处理
            String skipByteNum = field.getAttribute(new QName("skipByteNum"));
            if (null != skipByteNum && !"".equals(skipByteNum)) {
                int skipBytes = Integer.valueOf(skipByteNum);
                try {
                    in.skip(skipBytes);
                } catch (IOException exception) {
                    throw new RuntimeException(MessageFormat.format("跳过字节数{}异常", skipByteNum),
                            exception);
                }
            }
            ...
            //System.out.println("value=----->"+values[fieldIndex]);
        }
     ...
     * index number
     *
     * @param fields The name of the field array that is going to be placed into a
     *               new map object
